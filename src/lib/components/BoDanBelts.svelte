...
<div class="optionsc">
    ...
    <div class={clsx("optionc", active==='red' ? 'active':'')} on:click={()=> {if (active === 'red'){
        active = ''} else {
            active = 'red'
        }}
    }>
...
<div class={clsx("optionc", active==='stripe' ? 'active':'')} on:click={()=> {if (active === 'stripe'){
    active = ''} else {
        active = 'stripe'
    }}
}>
    ...
    <div class={clsx(active === 'stripe' ? "info p-12 text-surface-50" : "hidden")}>
        ...
        <table class="mt-8">
            ...
            <tr>
                <td>Knowledge:</td>
                <td>Poomsae name & meaning (all), 1Manual Questions 1-21, History of Taekwondo, Tents of Taekwondo, Philosophy of Taekwondo </td>
            </tr>
            <tr>
        ...
        <tr>
            <td>Regular assistance in classess (min. once per week)/td>
        </tr>
        <tr>
            <td>Pretest:</td>
...
<style>
    :root {
  --optioncColor1: red;
  --optioncColor2:  linear-gradient(to right, red 180px, black 200px, red 220px );
  ;
...
.optionc {
  ...
  min-width: 60px;
  margin: 10px;
  background: var(--optioncBackground, var(--defaultBackground, #E6E9ED));
...
.optionc:nth-child(1) {
  --defaultBackground: var(--optioncColor1);
}

.optionc:nth-child(2) {
    background-image: linear-gradient(90deg, #eb0e0e 23.68%, #000000 23.68%, #000000 50%, #eb0e0e 50%, #eb0e0e 73.68%, #000000 73.68%, #000000 100%) ; 
...
.optionc:nth-child(3) {
  --defaultBackground: var(--optioncColor3);
}

.optionc:nth-child(4) {
  --defaultBackground: var(--optioncColor4);
}

.option:nth-child(5) {
  --defaultBackground: var(--optionColor5);
}

.option:nth-child(6) {
  --defaultBackground: var(--optionColor6);
}
...
.optionc.active .shadow {
  box-shadow: inset 0 -120px 120px -120px black, inset 0 -120px 120px -100px black;
}

.optionc.active .label {
  bottom: 20px;
  left: 20px;
}

.optionc.active .label .info > div {
  left: 0;
  opacity: 1;
}
...
.optionc:not(.active) .label .info > div {
  left: 20px;
  opacity: 0;
}
